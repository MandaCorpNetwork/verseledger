...
const encodeString = (stream: Buffer, str: string): Buffer => {
  ...
  buff.write(str);
  const buffWithSize = Buffer.concat([
    handleLength(str, BType.STRING_SMALL, BType.STRING, BType.STRING_LARGE),
    buff,
  ]);
...
const encodeNumber = (stream: Buffer, num: number): Buffer => {
  ...
  return isDecimal
    ? encodeNumberDouble(stream, num)
    : encodeNumberLong(stream, BigInt(num));
};
