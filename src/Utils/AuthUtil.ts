...
export class AuthUtil {
  ...
  private static _isValidToken(token: JwtPayload | null) {
    ...
    if (token.iss !== 'api.verseledger.space') return false;
    if ((token.exp ?? 0) < Date.now()) return false;
    if (token.id !== token.sub) return false;
    return true;
  }
  private static _decodeToken(tokenString: string) {
    try {
  ...
  public static isValidToken(tokenString: string) {
