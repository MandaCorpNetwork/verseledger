...
export interface OAuth2Storage<Profiles extends string> {
   ...
   */
  set(req: any, name: Profiles, token: TOAuth2AccessToken): Promise<void>;
  /**
   * Get token from storage
   */
  get(req: any, name: Profiles): Promise<TOAuth2AccessToken | undefined>;
  /**
   * Delete token in storage (most likely a logout)
   */
  delete(req: any, name: Profiles): Promise<void>;
}
...
export interface OAuth2State<Profiles extends string> {
   ...
   */
  generate: (req: any, name: Profiles) => string;
  /**
   * Check if the state exists
   */
  check: (req: any, name: Profiles, state: string) => boolean;
}
...
const oauth2 = <Profiles extends string>({
  ...
  function buildLoginUri(name: string, external: boolean = true) {
    return buildUri(login as string, name, external);
  }

  function buildLogoutUri(name: string, external: boolean = true) {
    return buildUri(logout as string, name, external);
  }
  ...
  return (app: Elysia) => {
    (app as unknown as InternalOAuth2Elysia<Profiles>)
      ...
      // >>> AUTHORIZED <<<
      .get(authorized as string, async (req) => {
        const context = resolveProvider(req.params);
      ...
      // >>> LOGOUT <<<
      .get(logout as string, async (req) => {
        const context = resolveProvider(req.params);
